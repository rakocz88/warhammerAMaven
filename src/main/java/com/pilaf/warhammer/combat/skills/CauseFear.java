...
public class CauseFear implements SkillsEffect {
    @Override
    public Skills getSkillName() {
        return Skills.CAUSE_FEAR;
    }
    ...
    public double calculateLeadershipAfterEffect(Unit unit, Unit target, double leadership) {
        if (target.getSkillsList().contains(Skills.IMMUNE_TO_PSYCHOLOGY) || target.getSkillsList().contains(Skills.CAUSE_FEAR) || target.getSkillsList().contains(Skills.CAUSE_TERROR)){
            return leadership;
        } else {
            return leadership - 8;
        }
